...
class Net(nn.Module):
    ...
    def forward(self, feat, caption, caption_onehot, keep_rate):

        # clip sampling rate
        if keep_rate < 2: keep_rate = 2
        ...
        for step in range(self.max_length-1):
            ...
            if step > 0 and coin >= keep_rate:
                ...
                word_vec = self.data.IDs2VECs(word_ids)
                word_vec = Variable(torch.FloatTensor(word_vec)).cuda()
                h1,c1 = self.lstm_decode1(word_vec, (h1,c1))
                
            else:
                h1,c1 = self.lstm_decode1(caption[:,step,:], (h1,c1))

            one_hot_word = self.onehot(h1)
            word_loss    = loss(one_hot_word,caption_onehot[:,step+1])
    ...
    def inference(self, feat): 
        ...
        # <pad>
        padding_lstm1 = Variable(torch.zeros(1, self.hidden_size)).cuda();
        ...
        for step in range(self.max_length-1):          
            ...
            h1, c1 = self.lstm_decode1(previous_word, (h1, c1))
            # h2, c2 = self.lstm2(torch.cat((previous_word, h1), 1), (h2, c2))
            
            word_one_hot = self.onehot(h1)
...
def train(args):
    ...
    for epoch in range(epochs):
        for batch in range(batch_per_epoch):
            ...
            loss = model(feature, caption, caption_one_hot, 9-9/batch_per_epoch*batch)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            print ('===> epoch: {}, steps: {}, loss: {:.4f}, sampling rate: {:.4f}'.format(epoch+1, batch, loss.item(), batch/batch_per_epoch))
            if (batch+1)%5000 == 0 and save_dir is not None:
                torch.save(model.state_dict(), os.path.join(save_dir, 'model_schedule_sampling.pytorch-{}-{}'.format(epoch+1, batch+1)))
