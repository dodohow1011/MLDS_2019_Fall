...
class DataSet:
    def __init__(self, word2vec, training_features, training_label, testing_features, testing_label, batch_size=4):
        ...
        self.sentence_limit     = 30 # default max length of a sentence
        ...
        # remove sentences with OOV words
        self.check_labels()
    ...
    def next_batch(self):
        ...
        frames = [ np.load(v) for v in videos ]
    ...
    def test_batch(self):
        '''
        returns None when all of the data are tested
        '''
        ids    = self.testing_ids

        videos = [ os.path.join('MLDS_hw2_1_data/testing_data/feat', x+'.npy') for x in ids ]
