from simple_CGAN_v3 import Generator, Discriminator, NOISE_DIM, Embed
...
def sample_image(image_num, embed, gen, save_path):
    global SAVED_IMAGES
    attr = []
...
E_opt = optim.Adam(E.parameters(), lr=LEARNING_RATE)
...
for epoch in range(EPOCH):
    for step in range(steps_pre_epoch):
        ...
        # loss for real image
        real_d_out  = D(image, y.detach())
        real_d_loss = BCE_loss(real_d_out.squeeze(), real)

        # loss for fake image
        fake_d_out  = D(gen_imgs.detach(), y.detach())
        ...
        if step%100 == 0:
            ...
            sample_image(SAMPLE_NUM, E, G, 'epoch-{:03}-step{:04}'.format(epoch+1, step+1))
