...
conv2_channels_in    = int(conv1_channels_out * 2)
...
class Generator(nn.Module):
    def __init__(self, label_dim):
        ...
        self.deconv1_1 = nn.ConvTranspose2d(in_channels=NOISE_DIM, out_channels=deconv1_channels_out, kernel_size=4, stride=1, padding=0)
        self.bn1_1     = nn.BatchNorm2d(deconv1_channels_out)
        self.deconv1_2 = nn.ConvTranspose2d(in_channels=label_dim, out_channels=deconv1_channels_out, kernel_size=4, stride=1, padding=0)
        self.bn1_2     = nn.BatchNorm2d(deconv1_channels_out)
        self.deconv2   = nn.ConvTranspose2d(in_channels=deconv2_channels_in, out_channels=deconv2_channels_out, kernel_size=4, stride=2, padding=1)
        self.bn2       = nn.BatchNorm2d(deconv2_channels_out)
        self.deconv3   = nn.ConvTranspose2d(in_channels=deconv3_channels_in, out_channels=deconv3_channels_out, kernel_size=4, stride=2, padding=1)
        self.bn3       = nn.BatchNorm2d(deconv3_channels_out)
        self.deconv4   = nn.ConvTranspose2d(in_channels=deconv4_channels_in, out_channels=deconv4_channels_out, kernel_size=4, stride=2, padding=1)
        self.bn4       = nn.BatchNorm2d(deconv4_channels_out)
        self.deconv5   = nn.ConvTranspose2d(in_channels=deconv5_channels_in, out_channels=deconv5_channels_out, kernel_size=4, stride=2, padding=1)
...
class Discriminator(nn.Module):
    def __init__(self, label_dim):
        ...
        self.conv1_1 = nn.Conv2d(in_channels=conv1_channels_in, out_channels=conv1_channels_out, kernel_size=4, stride=2, padding=1)
        self.bn1_1   = nn.BatchNorm2d(conv1_channels_out)
        self.conv1_2 = nn.ConvTranspose2d(in_channels=label_dim, out_channels=conv1_channels_out, kernel_size=conv1_channels_out, stride=1, padding=0)
        self.bn1_2   = nn.BatchNorm2d(conv1_channels_out)
        self.conv2   = nn.Conv2d(in_channels=conv2_channels_in, out_channels=conv2_channels_out, kernel_size=4, stride=2, padding=1)
        self.bn2     = nn.BatchNorm2d(conv2_channels_out)
        self.conv3   = nn.Conv2d(in_channels=conv3_channels_in, out_channels=conv3_channels_out, kernel_size=4, stride=2, padding=1)
        self.bn3     = nn.BatchNorm2d(conv3_channels_out)
        self.conv4   = nn.Conv2d(in_channels=conv4_channels_in, out_channels=conv4_channels_out, kernel_size=4, stride=2, padding=1)
        self.bn4     = nn.BatchNorm2d(conv4_channels_out)
        self.conv5   = nn.Conv2d(in_channels=conv5_channels_in, out_channels=1, kernel_size=4, stride=1, padding=0)
    ...
    def forward(self, image, labelcode, drop_rate=0.2):
        ...
        y = F.leaky_relu(self.bn1_2(self.conv1_2(labelcode)), 0.2)
        x = torch.cat([x, y], 1)
